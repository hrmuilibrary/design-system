...
overlay currently has focus. Pass `false` as the last argument — e.g.
`useEscapeKey(onClose, false)` — to skip attaching the listener entirely —
useful when the overlay is closed.
