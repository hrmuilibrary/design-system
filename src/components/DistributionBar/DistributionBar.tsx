...
export const DistributionBar = forwardRef<HTMLDivElement, DistributionBarProps>(
  function DistributionBar(
    ...
    return (
      ...
      >
        <div
          className={cn('flex w-full overflow-hidden rounded-full bg-bg-subtle', sizeStyles[size])}
        >
          {segments.map((segment, index) => {
            ...
            return (
              <div
                ...
                className={cn(
                  'transition-[flex-basis] duration-700 ease-out',
                  colorStyles[segment.color],
                )}
              />
            );
          })}
              ...
              .map((segment, index) => (
                <li key={index} className="flex items-center justify-between gap-3 py-1.5">
                  <span className="flex min-w-0 items-center gap-2">
                    <span
                      className={cn('size-2 shrink-0 rounded-full', colorStyles[segment.color])}
                    />
